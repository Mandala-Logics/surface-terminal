    ...
    public class ListPanel : SurfacePanel, IReadOnlyList<SurfaceLine>
    {
        ...
        public override void Render(ISurface<ConsoleChar> surface, ulong frameNumber)
        {
            ...
            for (var x = firstVisible; x < lastVisible; x++)
            {
                ...
                if (_lines[x].State == SurfaceLineState.Selected)
                {
        ...
        protected override void OnKeyPressed(ConsoleKeyInfo keyInfo)
        {
            switch (keyInfo.Key)
            {
                ...
                case ConsoleKey.Enter:
                    if (_selectedLine != -1) _lines[_selectedLine].RequestStateChange(SurfaceLineState.Active);
                    break;
                case ConsoleKey.Delete:
                    if (_selectedLine != -1) _lines[_selectedLine].RequestStateChange(SurfaceLineState.Disabled);
                    break;
                default:
        ...
        private void SelectNextLine()
        {
            ...
            {
                if (_lines[b].RequestStateChange(SurfaceLineState.Selected) 
                    && _lines[_selectedLine].RequestStateChange(SurfaceLineState.Deselected))
                {
                    _selectedLine = b;
                    return;
                }
        ...
        public void Add(SurfaceLine item)
        {
            ...
            if (_selectedLine == -1 && item.RequestStateChange(SurfaceLineState.Selected))
            {
        ...
        public void RemoveAt(int index)
        {
            ...
            {
                _lines[index].RequestStateChange(SurfaceLineState.Deselected);
            }
